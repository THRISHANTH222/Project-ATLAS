...
interface SemanticGroup {
  type: 'heading' | 'section' | 'table' | 'list' | 'qa' | 'paragraph' | 'page_marker';
  start: number;
  end: number;
  text: string;
  pageNumber?: number;
...
export class SentenceChunker implements Chunker {
  ...
  public chunk(
    ...
    if (config.tokenizerStrategy === 'word') {
      if (maxLimit === 1000) maxLimit = 700;
      if (overlapLimit === 200) overlapLimit = 100;
    }

    const countWords = (s: string) => s.trim().split(/\s+/).filter(Boolean).length;
    const measureSize = (s: string) => Tokenizer.measureLength(s, config.tokenizerStrategy);
  ...
  private extractEnhancedMetadata(text: string, filename?: string): {
    documentType: string;
    department: string;
    keywords: string[];
    tags: string[];
  ...
  private splitIntoLinesWithOffsets(text: string): { text: string; start: number; end: number }[] {
  ...
  private parseSemanticGroups(
    ...
    const isHeading = (txt: string): boolean => {
      const trimmed = txt.trim();
      if (!trimmed) return false;
      if (/^(#{1,6})\s+.+$/.test(trimmed)) return true;
      if (trimmed.startsWith('**') && trimmed.endsWith('**') && trimmed.length < 100) return true;
      if (trimmed.length > 3 && trimmed.length < 80 && /^[A-Z0-9\s\-_:\(\)\.,]+$/.test(trimmed)) return true;
      return false;
    };

    const isSection = (txt: string): boolean => {
      const trimmed = txt.trim();
      if (!trimmed) return false;
      if (/^(Section|Chapter|Module|Part|Section\s+Title|Chapter\s+Title|Part\s+Title)\s+[A-Za-z0-9\.\-_]+/i.test(trimmed)) return true;
      return false;
    };

    const isTableLine = (txt: string): boolean => {
      const trimmed = txt.trim();
      return trimmed.startsWith('|') || /\|/g.test(trimmed) || /^\+[-+]*\+$/.test(trimmed);
    };

    const isListLine = (txt: string): boolean => {
      const trimmed = txt.trim();
      return /^([-*+•]|\d+[\.\)]|[a-zA-Z][\.\)])\s+/.test(trimmed);
    };

    const isQuestionLine = (txt: string): boolean => {
      const trimmed = txt.trim();
      return /^(Q\d+|Question\d*|Q:)\b/i.test(trimmed);
    };

    const getPageNumber = (txt: string): number | undefined => {
    ...
    while (i < n) {
      ...
      while (j < n) {
        ...
        if (getPageNumber(nextLine.text) !== undefined) break;
        if (isSection(nextLine.text)) break;
        if (isHeading(nextLine.text)) break;
        if (isTableLine(nextLine.text)) break;
        if (isListLine(nextLine.text)) break;
        if (isQuestionLine(nextLine.text)) break;
        j++;
      }

      const paragraphText = text.substring(lines[i].start, lines[j - 1].end);
      const sentences = SentenceSplitter.split(paragraphText);
  ...
  private chunkLongTextByWords(
    ...
    while (i < n) {
      ...
      while (j < n) {
        ...
        const candidateSize = Tokenizer.measureLength(candidateSlice, strategy);
      ...
      while (o > i) {
        ...
        const size = Tokenizer.measureLength(overlapText, strategy);
