...
const logger = new logger_1.Logger('PdfParser');
/**
...
class PdfParser {
    ...
    async parse(buffer, metadataFallback) {
        ...
        if (!buffer || buffer.length === 0) {
            throw new parser_errors_1.EmptyDocumentError('PDF content buffer is empty or missing.');
        }
        try {
            ...
            if (errMsg.includes('corrupt') ||
                ...
                throw new parser_errors_1.CorruptedFileError('PDF file is corrupted or formatted incorrectly.', error);
            }
            throw new parser_errors_1.UnexpectedParserError('An unexpected error occurred while parsing the PDF.', error);
        }
    }
}
