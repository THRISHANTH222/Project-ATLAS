...
function loadAndValidateConfig() {
    ...
    if (!apiKey && process.env.NODE_ENV === 'production') {
        throw new embedding_errors_1.EmbeddingConfigError('GEMINI_API_KEY or GOOGLE_API_KEY environment variable must be specified in production.');
    }
    ...
    if (isNaN(batchSize) || batchSize <= 0) {
        throw new embedding_errors_1.EmbeddingConfigError(`EMBEDDING_BATCH_SIZE must be a positive integer. Found: ${batchSizeStr}`);
    }
    if (isNaN(maxRetries) || maxRetries < 0) {
        throw new embedding_errors_1.EmbeddingConfigError(`EMBEDDING_RETRY_COUNT must be a non-negative integer. Found: ${maxRetriesStr}`);
    }
    if (isNaN(retryDelayMs) || retryDelayMs <= 0) {
        throw new embedding_errors_1.EmbeddingConfigError(`EMBEDDING_RETRY_DELAY_MS must be a positive integer. Found: ${retryDelayMsStr}`);
    }
    if (isNaN(timeoutMs) || timeoutMs <= 0) {
        throw new embedding_errors_1.EmbeddingConfigError(`EMBEDDING_TIMEOUT_MS must be a positive integer. Found: ${timeoutMsStr}`);
    }
    if (isNaN(concurrencyLimit) || concurrencyLimit <= 0) {
        throw new embedding_errors_1.EmbeddingConfigError(`EMBEDDING_CONCURRENCY must be a positive integer. Found: ${concurrencyStr}`);
    }
    return {
...
exports.embeddingConfig = loadAndValidateConfig();
